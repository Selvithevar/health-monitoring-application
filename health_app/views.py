...
import os
from health_project.settings import *
from .models import *
...
def index(request):
    #if you want to load this data in database uncomment this variable and run the code
    ...
    #
    # for key,values in symptoms_dict.items():
    #     Symptom.objects.create(name=key, value=values)
    sympton = Symptom.objects.all()
    return render(request,'index.html', {"sympton":sympton})



#load datasets
file_path = os.path.join(BASE_DIR, 'health_app', 'static', 'datasets')
file_path_pickle = os.path.join(BASE_DIR, 'health_app', 'static', 'models')
sym_des = pd.read_csv(os.path.join(file_path,'symtoms_df.csv'))
precautions = pd.read_csv(os.path.join(file_path,"precautions_df.csv"))
workout = pd.read_csv(os.path.join(file_path,"workout_df.csv"))
description = pd.read_csv(os.path.join(file_path,"description.csv"))
medications = pd.read_csv(os.path.join(file_path,"medications.csv"))
diets = pd.read_csv(os.path.join(file_path,"diets.csv"))

# load model
svc_model = os.path.join(file_path_pickle,'svc.pkl')
...
def predict(request):
    if request.method == "POST":
        symptoms = request.POST.dict().get("symptoms")
        print(symptoms,'<--symptons')
        ...
        for i in pre[0]:
            my_pre.append(i)
        context = {"dis_des":desc,"dis_pre":my_pre,"dis_med":med,"dis_die":die,"dis_wrkout":wrkout,"predicted_disease":predicted_disease}
