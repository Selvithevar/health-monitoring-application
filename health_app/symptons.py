# views.py in your Django app (e.g., health_app)
import os
import pandas as pd
...
def insert_symptoms_view(request):
    symptoms_dict = {
    ...
    for symptom, value in symptoms_dict.items():
        Symptom.objects.update_or_create(name=symptom, defaults={'value': value})

    return JsonResponse({'status': 'success', 'message': 'Symptoms data inserted successfully'})
